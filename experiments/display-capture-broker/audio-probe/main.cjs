...
function startTone() {
  if (process.platform === 'win32') {
    return spawn(
      'powershell',
      ['-NoProfile', '-File', path.join(__dirname, 'play-tone-loop.ps1'), TONE],
      { stdio: 'ignore' }
    );
  }
  ...
  return spawn(
    process.execPath,
    [path.join(__dirname, 'play-tone-loop.cjs'), player, TONE],
    { stdio: 'ignore', detached: process.platform === 'darwin' }
  );
}
