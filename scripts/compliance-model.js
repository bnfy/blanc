...
function assetComponents(policy) {
  ...
  for (const asset of policy.assets) {
    ...
    const component = {
      ...
      name: asset.name,
      scope: 'required',
      hashes: [{ alg: 'SHA-256', content: sha256File(asset.file) }],
...
function notices(runtime, policy) {
  const lines = [
    'BLANC THIRD-PARTY NOTICES',
    '',
    'Generated from package-lock.json and compliance/policy.json. Do not edit by hand.',
    'Blanc itself is proprietary/UNLICENSED; the notices below cover third-party material distributed with it.',
    '',
...
function createComplianceArtifacts() {
  ...
  return {
    ...
    files: {
      ...
      'compliance/THIRD_PARTY_NOTICES.txt': notices(runtime, policy),
    },
  };
}
