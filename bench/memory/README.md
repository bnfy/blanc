...
`baseline` (idle) is what makes the per-page column possible — it is added
automatically. `mixed` is the default. `adheavy` is the one that isolates what
network-layer blocking is worth, since blocked frames and scripts are never
instantiated at all. `scale` (20 pages) is where engines' process models and
per-tab overheads may diverge in ways ten tabs do not reveal — so any claim
about many tabs should **rest on that workload**, measured, rather than be
extrapolated from a smaller one.
