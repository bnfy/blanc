...
- **Desktop acceptance (`test/desktop`, `src/main/test-hook.js`):** `MAHJONG_TILE_COUNTS` gains the five new counts; the dock geometry assertion that today expects five circular ≥ 63.5 px controls inside the board frame is updated to six, at the 640×480 minimum viewport and the zoomed viewport the step already iterates; a new step opens the Records sheet at both viewports and asserts the sheet's `scrollWidth <= clientWidth` (no horizontal overflow) and that the card rect stays inside the frame's viewport, in the same style as the existing completion-card containment check (`readMahjongCompletionGeometry`).
