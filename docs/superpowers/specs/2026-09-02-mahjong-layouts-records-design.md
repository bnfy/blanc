...
- **Desktop acceptance (`test/desktop`, `src/main/test-hook.js`):** `MAHJONG_TILE_COUNTS` gains the five new counts; the ready step asserts six circular dock controls (`dockButtonCount === 6`); a new step drives the desktop rail through window heights that land the **embedded frame** (window minus the start page's ~132 px footer) on every tier edge — 611, 659, 660, 668 (the 1280×800 default), 720, 721 — and asserts six controls, `dockTop`/`dockBottom` containment inside the table, and the tier's control size and gap (611–659 px frame → 52 px/10 px, 660–720 → 56 px/14 px, ≥ 721 → 64 px/16 px; the tiers were measured against the Burst table, whose rack sits below the board); a new step opens the Records sheet at 1280×800 and 640×480 at zoom 1, then at zoom 1.5 and 1.25 through a new `setNewtabZoomFactor` hook (the page's own `webContents.setZoomFactor`, restored afterwards), and asserts eight rows, `scrollWidth <= clientWidth` (no horizontal overflow), the card rect inside the viewport, and focus returned to the dock control, in the same style as the existing completion-card containment check (`readMahjongCompletionGeometry`).
