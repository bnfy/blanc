...
| `bridge` | Bridge | 100 | 4 | 30×10 | ~14 | 100 tiles · narrow |
| `cross` | Cross | 86 | 4 | 30×16 | 17 | 86 tiles · layered |

Construction (verified by a throwaway probe against `isFreeAt`; the unit test in §3.3 re-verifies the shipped builders):
...
- **Bridge.** Two pylons (left origin `x 0`, right origin `x 22`): z0 4×5 (`x ox..ox+6`, `y 0..8`); z1 3×4 at half offsets (`x ox+1..ox+5`, `y 1..7`); z2 2×3 (`x ox+2..ox+4`, `y 2..6`); z3 `x ox+3` at `y 3, 5`. Span: z0 deck `x 9..19` step 2 × `y 3, 5` (12 tiles, one-tile gap from each pylon so the deck ends are free); z1 cables `x 10..18` at `y 4` (5); z2 `x 12, 14, 16` at `y 4` (3). 40 + 24 + 12 + 4 + 20 = 100.
...
- **Idempotent counting.** `mergeEvents()` re-applies every retained completion event on every `read()`, which is safe for best-of records but not for a counter. So counting lives in `mergeEvents`, not `applyResult`: for each retained event whose id is not in `countedEvents`, increment `cleared[mode][layoutId]` and add the id. After pruning to `MAX_RECORD_EVENTS`, drop from `countedEvents` any id no longer among retained events (a pruned event can never be re-applied). The list is therefore bounded at 128 ids (~5 KB). Only `completed: true` events with a valid layout revision count. Daily and random completions both count.
...
- **Concurrency.** Two tabs finishing at once each write their own event; the aggregate is a cache rebuilt from events, so the only double-count window is an event counted by one tab and pruned by another within the same instant at 128+ retained events. Accepted.
...
- **Page (source-guard style, as the suite already does):** dock button and sheet ids exist, `activeModal()` lists the sheet, `R` shortcut wired, `paintRecords` uses `S.recordsSummary`, eight layout cards in `LAYOUT_IDS` order, grid columns rule present.
...
- `countedEvents` grows the aggregate by up to ~5 KB. Well inside the localStorage budget the game already uses.
