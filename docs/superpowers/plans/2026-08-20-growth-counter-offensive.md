...
- **Open decision, owner's call:** the FAQ and README describe Blanc as *source-available* under `UNLICENSED`. That is accurate but leaves a soft edge — the README invites `npm install && npm start` while the package grants no licence at all. Adding an explicit proprietary source-available licence (permitting inspection and personal builds, withholding redistribution) would remove the ambiguity. **This plan does not choose a licence**; it uses precise wording instead. Raise it before launch if you want it settled.
