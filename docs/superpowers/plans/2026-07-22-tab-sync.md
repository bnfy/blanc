...
- Produces:
  ...
  - `applyBudget(devices, ownId, {maxBytes?}) -> devices` — trims own tabs first, then drops stalest others.
...
test('applyBudget: trims own tabs first, then drops stalest other devices', () => {
  ...
  assert.ok(m.canonical(out).length <= 120 * 1024);
 ...
 * the combined encrypted map, so bound the plaintext here. Trim our own tab
 * list from the end first; only then drop the stalest OTHER devices — they
 * resurrect from their own next push. Input is not mutated. */
function applyBudget(devices, ownId, { maxBytes = BUDGET_BYTES } = {}) {
  const out = { ...devices };
  const over = () => canonical(out).length > maxBytes;
  if (!over()) return out;
...
module.exports = {
  ...
  devicesEqual, applyBudget, displayDevices,
};
```
...
- Produces (consumed by sync.js in Task 6 and main.js in Task 8):
  - `setSnapshotProvider(fn)` — `fn: () => ({ tabList, groups })` with live main.js state.
  - `onChanged(fn)` — fired when the publishable snapshot's fingerprint changes.
...
const setSnapshotProvider = (fn) => { snapshotProvider = fn; };
 ...
 * loading-flag flips schedule nothing (spec §6). The first call is the
 * startup baseline, not a change (sync-on-launch already covers it). */
function noteTabsChanged() {
  if (!snapshotProvider) return;
  const fp = model.canonical(currentSnapshot());
  if (fp === lastFingerprint) return;
  const isBaseline = lastFingerprint === null;
  lastFingerprint = fp;
...
function exportForSync(ctx) {
  ...
  const now = Date.now();
  s.update((d) => {
    const own = model.ownEntryFor({
      syncTabs: ctx.syncTabs,
      prev: d.devices[ctx.deviceId] ?? null,
      snapshot: snapshotProvider ? currentSnapshot() : null,
      name: ctx.deviceName,
      platform: ctx.platform,
      now,
    });
    if (own) d.devices[ctx.deviceId] = own;
    else delete d.devices[ctx.deviceId];
    // mergeDevices against {} applies the 30-day prune to our own cache.
    d.devices = model.applyBudget(model.mergeDevices(d.devices, {}, { now }), ctx.deviceId);
  });
  return { devices: s.data.devices };
}

function mergeFromSync(remote, ctx) {
  rebind(ctx.accountId).update((d) => {
...
function onSyncDisabled() {
  ensureStore().update((d) => { d.accountId = ''; d.devices = {}; });
}

module.exports = {
  setSnapshotProvider, onChanged, noteTabsChanged,
...
**(f)** Start-page hook — in the `setupPages({ ... startPage: { ... } })` block (~line 2070), add to `startPage`:
...
**Interfaces:**
- Consumes: `chrome:remote-tabs-list` (Task 8) returning `[{deviceId, name, platform, updatedAt, tabs: [{url, title, groupId, pinned}], groups: [{id, name}]}]`.
- Produces: user-visible panel sections + switcher `kind: 'remote'` results.

- [ ] **Step 1: preload.js**

Next to `listFavorites:` add:
...
**Interfaces:**
- Consumes: `hooks.startPage.remoteDevices` (Task 8).
- Produces: `pages:start:data` response gains `remoteDevices`.
...
window.bowserPages?.start.data().then(({ groups, blockedThisWeek, remoteDevices }) => {
  ...
  // Tab sync: other devices' tabs, read-only — clicking navigates the
  // current tab, same as favorites above. Renders only when snapshots
  // exist so the ledger stays quiet otherwise (spec §2).
...
```bash
git add src/main/pages.js src/renderer/pages/newtab.html src/renderer/pages/newtab.js
git commit -m "start page: on-your-other-devices section (tab-sync task 11)"
```
...
- Modify: `docs/superpowers/specs/2026-07-07-profile-sync-design.md` (§4 table)
...
In the **Profile Sync** paragraph, change the first sentence's scope phrase from "cross-device sync of **Favorites and settings**" to "cross-device sync of **Favorites, settings, and (opt-in per device) open tabs**", and append to the end of the paragraph:
...
```bash
git add spec/features.md spec/parity-matrix.md spec/acceptance/sync.feature spec/acceptance/index.md docs/superpowers/specs/2026-07-07-profile-sync-design.md CLAUDE.md
git commit -m "spec: F27 tab sync — features, parity matrix, acceptance backlog, CLAUDE.md (tab-sync task 12)"
```
...
4. Quit. Simulate a second device on the same machine: delete `sync.json` and `tab-sync.json` from the dev userData dir (this discards the local `deviceId`, so the app becomes a fresh device identity), relaunch, and re-enable sync with the SAME handle/passphrase, leaving sharing OFF. The first identity's published entry now appears as a remote device.
