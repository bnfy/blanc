...
**Type consistency.** `--dl-progress` is named identically in Tasks 9's JS and CSS. `.ghead-kbd` is introduced in Task 2 and used nowhere else. `#pillShieldCount` and the `shield-quiet`/`shield-off` classes in Task 8 match the existing `renderer.js:411-412` toggles. `ICONS.download` in Task 9 matches the `ICONS` object at `overlay.js:109`; `ICONS.bookmark` is deliberately **not** added (Task 10 Step 2 skips it — the footer glyph is authored in `overlay.html`, so the entry would be unused).
