...
Packaged v1.16.2 → `970bd6f9` migration preserved the exact pre-migration workspace backup with owner-only permissions, retained the binding after restart, and left a future-format file byte-identical through rejected mutations and shutdown. See packaged-migration-results.txt. [Parity guards](https://github.com/bnfy/blanc/actions/runs/34782219429) (unit, substrate, acceptance wiring, OAuth and tab handoff) and [CodeQL](https://github.com/bnfy/blanc/actions/runs/34782219424) passed for that earlier product revision. Current-revision CI and native validation remain pending after push.
