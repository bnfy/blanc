...
function validCurrentRun(value) {
  return !!(value && typeof value === 'object' && safeTime(value.startedAt) !== null);
}
...
function createCrashLedger(store, { now = Date.now } = {}) {
  ...
  function recordRenderer({ surface = 'unknown', reason, exitCode } = {}) {
    return store.updateAndFlush((data) => appendEvent(data, {
      at: now(), kind: 'renderer', surface, reason, exitCode,
    }));
  }

  function recordChildProcess({ type, reason, exitCode } = {}) {
    return store.updateAndFlush((data) => appendEvent(data, {
      at: now(), kind: 'child-process', processType: type, reason, exitCode,
    }));
  }
  ...
  const snapshot = () => normalizeEvents(store.data.events).map((event) => ({ ...event }));
  return {
    startSession,
    endSession,
    hasActiveSession: () => validCurrentRun(store.data.currentRun),
    hasPendingRecovery: () => store.data.recoveryPending === true,
    recordRenderer,
    recordChildProcess,
    resolveRecovery,
    clear,
...
function buildDiagnosticsReport({ generatedAt, appInfo = {}, systemInfo = {}, events = [] } = {}) {
  const crashes = normalizeEvents(events).map((event) => {
    ...
    if (event.kind === 'unclean-exit') projected.previousStartedAt = isoTime(event.previousStartedAt);
    return projected;
  });
  return {
    schemaVersion: 1,
    ...
    crashLedger: { eventCount: crashes.length, events: crashes },
    privacy: {
      ...
      excluded: [
        ...
        'quiet-tab and reopen-closed-tab snapshots',
      ],
    },
  };
}
