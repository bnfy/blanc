...
function createCertificateObserver() {
        ...
        if (hostname && request?.verificationResult === 'OK') {
          ...
          byHost.set(hostname, {
            certificate: sanitizeCertificate(
              request.validatedCertificate ?? request.certificate
            ),
            isIssuedByKnownRoot: request.isIssuedByKnownRoot === true,
          });
          if (byHost.size > MAX_CERTIFICATE_RECORDS) {
            byHost.delete(byHost.keys().next().value);
          }
        } else if (hostname) {
          byHost.delete(hostname);
        }
      } finally {
        // Preserve Chromium's complete verifier, including Certificate
        // Transparency. 0 would accept and disable CT; -2 would reject all.
        callback(-3);
      }
    });
  }
  ...
  function get(session, url) {
    try {
      ...
      return records.get(session)?.get(parsed.hostname.toLowerCase()) ?? null;
    } catch {
      return null;
    }
  }
...
function buildSiteInfo(url, {
  ...
  if (parsed.protocol === 'http:' && isLoopbackHost(parsed.hostname)) {
    return {
      ...
      summary: 'This loopback address stays on this device.',
    };
  }
  if (parsed.protocol === 'http:') {
    return {
      ...
      summary: 'Information sent to this site can be read or changed in transit.',
    };
  }
  if (parsed.protocol === 'blanc:') {
    return {
      ...
      state: 'internal',
      title: 'Blanc page',
      summary: 'This page is part of Blanc.',
    };
  }
  return {
    ...base,
    ...
    summary: 'This page does not use an HTTP connection.',
  };
}
