...
test('runtime download rejects incomplete, foreign, wrong-workflow and pull-request builds', () => {
  const valid = { path: workflow, event: 'workflow_dispatch', conclusion: 'success', head_repository: { full_name: repo }, head_sha: 'a'.repeat(40) };
  assert.equal(validateRuntimeRun(valid, { repo, workflow }), valid.head_sha);
  for (const patch of [{ conclusion: null }, { conclusion: 'failure' }, { event: 'pull_request' },
    { path: '.github/workflows/unrelated.yml' }, { head_repository: { full_name: 'other/blanc' } }, { head_sha: 'not-a-commit' }]) {
    assert.throws(() => validateRuntimeRun({ ...valid, ...patch }, { repo, workflow }));
  }
});
