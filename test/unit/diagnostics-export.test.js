...
test('diagnostics export atomically writes owner-only JSON and removes its temporary file', async (t) => {
...
test('a failed export leaves the destination untouched and cleans its temporary file', async () => {
  ...
  await assert.rejects(writeDiagnosticsFile('/report.json', { schemaVersion: 1 }, {
    fileSystem, nonce: () => 'failed',
  }), /disk full/);
