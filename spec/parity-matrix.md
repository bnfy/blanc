...
| F39 | Bring Your Tabs (AI-assisted tab migration) | PLANNED | PLANNED | PLANNED | An explicitly selected bookmarks folder becomes a reviewed, preview-ordered set of quiet tabs plus optional named groups; deterministic folder suggestions always work, only the first imported tab loads, immediate source subfolders are preserved in Favorites, and tab/group apply is atomic before an idempotent Favorites write. Migration and group suggestions are free; on-device semantic assistance and Patron workspace save are additive. | D22 |
