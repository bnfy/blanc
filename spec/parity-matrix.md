...
| F3 | Tab groups | SHIPPED | PLANNED | PLANNED | Names not colors (lowercase mono). Group exists only while non-empty. Pill renders standalone pins + the active section and counts every omitted tab in `+N`. Same create/move/ungroup/close-group actions. | — |
...
| F7 | Slash commands | SHIPPED | PARTIAL | PLANNED | Shared commands keep identical names + hints; `/1password` is available only on macOS under D26. | D7, D26 |
...
| F14 | Settings | SHIPPED | PARTIAL | PLANNED | Same shared keys, defaults, and validation (search engine, device-local `searchSuggestions`, adblock, home page, theme, app icon, exceptions, usage measurement, supporter); macOS desktop alone exposes F38's device-local 1Password toggle/account routing value. | D5, D6, D26 |
...
| F16 | Internal `blanc://` pages | SHIPPED | PARTIAL | PLANNED | newtab ledger, favorites, history, downloads, settings, shortcuts, error — same content/copy; utility pages present as a transient chrome surface (desktop: sheet), never tabs. | — |
| F17 | Patron & app icons | SHIPPED | PLANNED | PLANNED | Four current colorways (Sunrise, Sunrise Dark, Paper, Ink); Patron unlocks Named Workspaces, not icon variants. | D5, D6 |
...
| F20 | HTTP authentication policy | SHIPPED | PLANNED | PLANNED | No platform shows its own credentials dialog for HTTP Basic/Digest or proxy challenges; the challenged request fails, and the page can observe the 401/407 response where the browser permits. | — |
| F21 | Telemetry (bounded usage measurement) | SHIPPED | PLANNED | PLANNED | A fresh profile must commit its on/off choice before any event; the choice is presented on by default and remains device-local. When enabled, packaged desktop builds send one launch event plus once-per-session fixed metrics for the first Mahjong move and each rendered start-page layout. Private tabs and browsing/game content are excluded. | — |
...
| F35 | Start page layouts | SHIPPED | PLANNED | PLANNED | Four layouts (ledger/billboard/shelf/tally), chosen by a synced setting with an in-page switcher; Billboard ranks local history by hostname, shows full local page titles, reuses locally captured favicon pixels, and keeps dismissals in localStorage only; per-day blocked counts feed the tally chart; every footer opens Mahjong in its own managed tab; no horizontal overflow at any width. | — |
| F36 | First-run onboarding | SHIPPED | PLANNED | PLANNED | A six-step walkthrough shown once to fresh profiles (default browser, import, island, blocking, privacy, theme); privacy consent gates its network features, import keeps F30's explicit discovery, and both successful-import and skip paths can hand off to F40 without making full import a prerequisite. Replayable from Settings. Default-browser registration and live profile import are desktop surfaces (D22). | D22 |
...
| F38 | Fill a login from 1Password | PARTIAL | N/A | N/A | macOS-only, off-by-default explicit bridge to the user's installed 1Password app/account; Windows/Linux expose no surface and cannot start the broker; saved-website behavior and a revalidated login form bound the fill; no Blanc credential store, background lookup, renderer IPC, persistence, sync, or telemetry. | D26 |
...
narrow macOS alternative explicitly:
