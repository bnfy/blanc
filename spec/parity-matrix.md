...
| F35 | Start page layouts | SHIPPED | PLANNED | PLANNED | Five layouts (ledger/billboard/shelf/tally/mahjong), chosen by a synced setting with an in-page switcher; Billboard ranks local history by hostname, shows full local page titles, reuses locally captured favicon pixels, and keeps dismissals in localStorage only; per-day blocked counts feed the tally chart; Mahjong embeds the local solitaire game; no horizontal overflow at any width. | — |
